...
class Trainer:
    def __init__(self, options, train_loader, val_loader, save_folder, batch_size, use_affinity):
        ...
        self.models['control_depth'] = networks.Control_DepthDecoder(scales = self.opt.scales)
        self.models['control_depth'].to(self.device)
        self.parameters_to_train += list(self.models['control_depth'].parameters())
        ...
        self.backproject_depth_single = BackprojectDepth(1, height = 192, width = 640)
        ...
        self.project_3d_single = Project3D(1, height = 192, width = 640)
        ...
        self.file_path = os.path.join(self.record_saves, 'finetuning_A9_dual_pos.txt')
    ...
    def set_train(self):
        ...
        self.models['control_depth'].train()
    ...
    def set_eval(self):
        ...
        self.models['initial_teacher'].eval()
        self.models['encoder_teacher'].eval()
        self.models['control_depth'].eval()
    ...
    def load_model(self):
        ...
        path = 'D:\\depth_estimation_implementation\\all_saves\\weight_saves\\finetuned_A5'
        path1 = 'D:\\depth_estimation_implementation\\all_saves\\weight_saves\\finetuning_A9_dual_pos'
        ...
        depth_name = 'Control_Depth' + str(ep2) + '.pth'
        self.models['control_depth'].load_state_dict(torch.load(os.path.join(path1, depth_name)), strict=False)
        
        
        for param in list(self.models['initial'].parameters()) + list(self.models['encoder'].parameters()) + list(self.models['depth'].parameters()) + list(self.models['pose'].parameters()):
    ...
    def save_model(self):
        path = 'D:\\depth_estimation_implementation\\all_saves\\weight_saves\\finetuning_A9_dual_pos'

        depth_name = 'Control_Depth' + str(self.epoch) + '.pth'
        
        torch.save(self.models['control_depth'].state_dict(), os.path.join(path, depth_name))
    ...
    def val(self):
        ...
        self.batch_size = self.backup_batch
        self.set_train()
        self.val_mode = False
    ...
    def process_batch_val(self, inputs, outputs):
        ...
        if self.opt.pose_model_type == 'shared':
            ...
            # print('Encoder out: ', all_features[0].shape)
            # all_features = [torch.split(f, self.opt.batch_size) for f in all_features]
            all_features = [torch.split(f, 1) for f in all_features]
            # print('-- all features shape: ', len(all_features), len(all_features[0]))
            ...
            if outputs != None:
                ...
                prev_maps = self.models['depth'](features[1])#, control_feats
            
            
            control_feats = self.models['control_depth'](features[0], prev_maps)
            outputs = self.models['depth'](features[0], control_feats)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == 'shared':
            ...
            all_features = self.models['initial'](all_colour_aug.cuda().float())
            all_features = self.models['encoder'](all_features)
            print('-- to buffer shape: ', all_features)
            ...
            outputs_m1 = self.models['depth'](features[1])
            control_outputs = self.models['control_depth'](features[0], outputs_m1)
            outputs = self.models['depth'](features[0], control_outputs)
            
            # control_outputs_p1 = self.models['control-depth'](features(1), outputs)
            # outputs_p1 = self.models['depth'](features[1], control_outputs_p1)
    ...
    def generate_images_pred_single(self, inputs, outputs):
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(init_arr):
                ...
                cam_points = self.backproject_depth_single(depth, inputs[("inv_K")])
                pix_coords = self.project_3d_single(cam_points, inputs[("K")], T)
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(init_arr):
                ...
                if frame_id == -2:
                    ...
                    control_feats = self.models['control-depth'](features[1], prev_maps)
                    outputs_second = self.models['depth'](features[1], control_feats)
    ...
    def compute_losses_single(self, inputs, outputs):
        ...
        # print('total loss requires_grad: ', total_loss.requires_grad)
    ...
    def compute_losses(self, inputs, outputs, outputs_second, outputs_teacher, features, teacher_features, buffered_features):
        ...
        for scale in self.opt.scales:
            ...
            losses["loss/{}".format(scale)] = rp2#loss
